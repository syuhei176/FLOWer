(function(){
	function Memory(id, diagram, x, y, inputs, outputs, shape, logic_function, name) {
		...
		this.memories = [];
		if(typeof logic_function == "string" || this.logic_function == undefined) {
	...
	Memory.prototype.dblclick = function() {
		var self = this;
		var input_html = [];
		var output_html = [];
	...
	Memory.prototype.set_logic = function(inputs, script) {
		
	}
	
	Memory.prototype.clone = function(id) {
		var inputs = [];
		var outputs = [];
		for(var key in this.input) {
			inputs.push({
		...
		return new retro.Memory(id, this.diagram, this.bound.x, this.bound.y, inputs, outputs, {}, this.logic_function_script);
	}

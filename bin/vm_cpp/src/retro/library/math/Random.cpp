...
{
HX_STACK_PUSH("Random::new","retro/library/math/Random.hx",17);
{
	HX_STACK_LINE(18)
	this->name = HX_CSTRING("Random");
	HX_STACK_LINE(19)
...
hx::Object *Random_obj::__ToInterface(const hx::type_info &inType) {
	if (inType==typeid( ::retro::core::JobComponent_obj)) return operator ::retro::core::JobComponent_obj *();
	return super::__ToInterface(inType);
}

::String Random_obj::getModuleName( ){
	HX_STACK_PUSH("Random::getModuleName","retro/library/math/Random.hx",36);
	HX_STACK_THIS(this);
	HX_STACK_LINE(36)
	return HX_CSTRING("math.Random");
}


HX_DEFINE_DYNAMIC_FUNC0(Random_obj,getModuleName,return )

Void Random_obj::onInputRecieved( ::retro::core::Params params,Dynamic cb){
{
		HX_STACK_PUSH("Random::onInputRecieved","retro/library/math/Random.hx",25);
		HX_STACK_THIS(this);
		...
		HX_STACK_LINE(32)
		result->set(HX_CSTRING("output"),(v->getValue() * ::Math_obj::random()));
		HX_STACK_LINE(33)
		cb(result).Cast< Void >();
	}
return null();
...
HX_DEFINE_DYNAMIC_FUNC2(Random_obj,onInputRecieved,(void))
...
{
	hx::Static(__mClass) = hx::RegisterClass(HX_CSTRING("retro.library.math.Random"), hx::TCanCast< Random_obj> ,sStaticFields,sMemberFields,
